...
function ProdsLayout() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      let { data } = await ProdsAPI.getListCategory()
      let rs = []
      for (const property in data) {
        if (property !== 'NH' && data[property] && data[property].length > 0) {
          let obj = {
            ...data[property][0],
            children: data[property].filter((x, i) => i !== 0)
          }
          rs.push(obj)
        }
      }
  ...
  useEffect(() => {
    if (data) {
      let index = data.findIndex(x => x.ID === Number(CateID))

...
function FreesPage(props) {
  ...
  const { data, isLoading, isFetching, refetch, fetchNextPage } =
    useInfiniteQuery({
      queryKey: ['ListProdsFees', { filters }],
    ...
    if (!ReadCate?.hasRight) {
      ...
      if (recheck?.data?.error) {
        Swal.fire({
          ...
          title: 'Xóa phụ phí ?',
          html: `<span class="text-danger">Phụ phí ${item?.Title} đã phát sinh giao dịch. Bạn không thể xóa vĩnh viễn mà chỉ có thể chuyển về chế độ Ngừng Kinh doanh.</span>`,
          icon: 'warning',
        ...
        Swal.fire({
          ...
          title: 'Xóa phụ phí ?',
          html: `Bạn có chắc chắn muốn xoá phụ phí <span class="text-primary font-medium">${item?.Title}</span>? Hành động này không thể được hoàn tác.`,
          icon: 'warning',
          showCancelButton: true,
      ...
      <div className="flex items-end justify-between mb-5">
        <div>
          <div className="text-3xl font-bold dark:text-white">
            <span className="hidden md:block">Quản lý phụ phí</span>
            <span className="md:hidden">Phụ phí</span>
          </div>
          <div className="mt-1.5">
            Quản lý thêm mới, chỉnh sửa & xoá phụ phí
          </div>
        </div>

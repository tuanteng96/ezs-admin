...
function MoneyCardPage(props) {
  ...
  const { data, isLoading, isFetching, refetch, fetchNextPage } =
    useInfiniteQuery({
      queryKey: ['ListProdsMoneyCard', { filters }],
    ...
    if (!ReadCate?.hasRight) {
      ...
      if (recheck?.data?.error) {
        Swal.fire({
          ...
          title: 'Xóa thẻ tiền ?',
          html: `<span class="text-danger">Thẻ tiền ${item?.Title} đã phát sinh giao dịch. Bạn không thể xóa vĩnh viễn mà chỉ có thể chuyển về chế độ Ngừng Kinh doanh.</span>`,
          icon: 'warning',
        ...
        Swal.fire({
          ...
          title: 'Xóa thẻ tiền ?',
          html: `Bạn có chắc chắn muốn xoá thẻ tiền <span class="text-primary font-medium">${item?.Title}</span>? Hành động này không thể được hoàn tác.`,
          icon: 'warning',
          showCancelButton: true,
      ...
      <div className="flex items-end justify-between mb-5">
        <div>
          <div className="text-3xl font-bold dark:text-white">
            <span className="hidden md:block">Quản lý thẻ tiền</span>
            <span className="md:hidden">Thẻ tiền</span>
          </div>
          <div className="mt-1.5">
            Quản lý thêm mới, chỉnh sửa & xoá thẻ tiền
          </div>
        </div>
          ...
          <Tooltip
            ...
            overlay={
              <div className="rounded px-0 py-2 border-0 w-[200px] bg-white shadow-lg shadow-blue-gray-500/10 dark:bg-site-aside dark:shadow-dark-shadow">
                ...
                <PickerSettingsCommission
                  Type={{
                    label: 'Tất cả thẻ tiền',
                    value: MenuActive?.ID
                  }}
                  invalidateQueries={['ListProdsMoneyCard']}
                >
                  {({ open }) => (
                    <button
                      type="button"

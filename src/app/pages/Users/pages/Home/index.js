...
function Home(props) {
  ...
  const ListGroups = useQuery({
    queryKey: ['GroupRolesSidebar', filters.StockIDs],
    queryFn: async () => {
  ...
  return (
    <div className="relative flex h-full bg-white dark:bg-dark-app">
      <PerfectScrollbar
        ...
        className="w-56 h-full px-3 py-4 border-r border-separator"
      >
        <div className="font-bold font-inter text-[17px] py-2 px-4 dark:text-white">
          Phân nhóm
        </div>
        <div className="flex-col hidden gap-px mt-1 lg:flex">
          {ListGroups.isLoading && (
            <div>
              {Array(5)
                .fill()
                .map((_, i) => (
                  <div
                    className="h-3.5 bg-gray-200 rounded-full dark:bg-gray-700 w-full mb-3 last:mb-0"
                    key={i}
                  ></div>
                ))}
            </div>
          ...
          {!ListGroups.isLoading && (
            <>
              {ListGroups?.data &&
                ListGroups?.data.map((gr, i) => (
                  <div
                    key={i}
                    className={clsx(
                      ' cursor-pointer block px-4 py-3 text-[15px] rounded-md font-medium hover:bg-primarylight hover:text-primary dark:hover:bg-dark-light transition mt-1 dark:text-white',
                      filters.GroupIDs.includes(gr.value) &&
                        'bg-primarylight text-primary dark:bg-dark-light cursor-pointer'
                    )}
                    onClick={e => {
                      if (filters.GroupIDs.includes(gr.value)) {
                        ...
                        setFilters(prevState => ({
                          ...prevState,
                          GroupIDs: [...prevState.GroupIDs, gr.value]
                        }))
                      }
                    }}
                  >
